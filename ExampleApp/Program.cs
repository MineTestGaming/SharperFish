...
class Program
{
    static void Main(string[] args)
    {
        ...
        StreamReader reader =
            new StreamReader(client.PostAsync(requestKeyUri, requestContent).Result.Content.ReadAsStream());
        ...
        StringContent content =
            new StringContent(JsonSerializer.Serialize(loginInfo), Encoding.UTF8, "application/json");
        ...
        StringContent encryptedProfileContent = new StringContent(JsonSerializer.Serialize(encryptedProfileFetch),
        ...
        // Set the privacy as the user want
        jwtPubKeyResponse = client.PostAsync(requestKeyUri, requestContent);
        jwtPubKeyReader = new StreamReader(jwtPubKeyResponse.Result.Content.ReadAsStream());
        ...
        StringContent profileContent = new StringContent(JsonSerializer.Serialize(bundle), Encoding.UTF8, "application/json");
        var updateProfile = client.PostAsync($"{serverAddress}/api/profile/set", profileContent);
